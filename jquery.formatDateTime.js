 ...
 *
 * Refactored by Miloš Levačić <milos.levacic@creitive.rs>
 */
(function( $ ) {
	...
	var formatDateTime = function( format, date, settings ) {
		...
		/**
		 * Checks whether a format character is doubled, in which case, it
		 * should be padded with leading zeros most of the time.
		 * @param  {String}  match
		 * @return {Boolean}
		 */
		...
		/**
		 * Formats a number, including leading zeros when required.
		 * @param  {String} match
		 * @param  {String} value
		 * @param  {Number} len
		 * @return {String}
		...
		/**
		 * Formats a short or long name, as required.
		 * @param  {String} match
		 * @param  {String} value
		...
		for ( iFormat = 0; iFormat < format.length; iFormat++) {
			if ( literal ) {
				...
				switch ( format.charAt( iFormat ) ) {
					case "a":
						if ( date.getHours() < 12 ) {
							output += settings.ampmNames[0];
						} else {
							output += settings.ampmNames[1];
						}
						break;
					...
					case "D":
						output += formatName( "D", date.getDay(), settings.dayNamesShort, settings.dayNames );
						break;

					case "o":
						var end = new Date( date.getFullYear(), date.getMonth(), date.getDate() ).getTime();
						var start = new Date( date.getFullYear(), 0, 0 ).getTime();
						var dayOfYear = Math.round( (end - start) / 86400000 );

						output += formatNumber( "o", dayOfYear, 3 );
						break;

					case "g":
						var hour = (date.getHours() % 12) || 12;
						output += formatNumber( "g", hour, 2 );
						break;
					...
					case "M":
						output += formatName( "M", date.getMonth(), settings.monthNamesShort, settings.monthNames );
						break;
